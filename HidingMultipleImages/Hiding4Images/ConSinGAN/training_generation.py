...
def train(opt, opt_2, opt_3, opt_4, new_opt):
    ...
    real_4 = functions.read_image(opt_4)
    real_4 = functions.adjust_scales2image(real_4, opt_4)
    opt.reals_4 = functions.create_reals_pyramid(real_4, opt_4)
    ...
    for scale_num in range(opt.stop_scale+1):
        ...
        fixed_noise, fixed_noise_2, fixed_noise_3, fixed_noise_4, noise_amp, noise_amp_2, noise_amp_3, noise_amp_4, generator, d_curr = train_single_scale(d_curr, generator, reals, opt, scale_num, writer)
        ...
        torch.save(fixed_noise_4, '%s/fixed_noise_4.pth' % (opt.out_))
        ...
        torch.save(noise_amp_4, '%s/noise_amp_4.pth' % (opt.out_))
        del d_curr
    writer.close()
    return
...
def train_single_scale(netD, netG, reals, opt, depth, writer):
    ...
    if depth == 0:
        ...
        if opt.train_mode == "generation" or opt.train_mode == "animation":
            ...
            z_opt_3 = functions.generate_noise([opt.nfc,
                                                ...
                                                reals_shapes[depth][3] + opt.num_layer * 2],
                                               device=opt.device)
            z_opt_4 = functions.generate_noise([opt.nfc,
                                                reals_shapes[depth][2] + opt.num_layer * 2,
                                                reals_shapes[depth][3] + opt.num_layer * 2],
                                               device=opt.device)
        else:
    ...
    opt.fixed_noise_4.append(z_opt_4.detach())
    ...
    if depth == 0:
        ...
        z_reconstruction_4 = netG(opt.fixed_noise_4, reals_shapes, opt.noise_amp_4)
    ...
    for iter in _iter:
        ...
        if alpha != 0:
            ...
            rec_4 = netG(opt.fixed_noise_4, reals_shapes, opt.noise_amp_4)

            rec_loss = alpha * loss(rec, real)
            rec_loss_2 = alpha * loss(rec_2, real_2)
            rec_loss_3 = alpha * loss(rec_3, real_3)
            rec_loss_4 = alpha * loss(rec_4, real_4)

        else:
            rec_loss = 0

        netG.zero_grad()
        errG_total = errG + rec_loss + rec_loss_2 + rec_loss_3 + rec_loss_4
        errG_total.backward()
        ...
        if iter % 500 == 0 or iter+1 == opt.niter:
            ...
            functions.save_image('{}/reconstruction_{}_4.png'.format(opt.outf, iter + 1), rec_4.detach())
    ...
    return opt.fixed_noise, opt.fixed_noise_2, opt.fixed_noise_3, opt.fixed_noise_4, opt.noise_amp, opt.noise_amp_2, opt.noise_amp_3, opt.noise_amp_4, netG, netD
