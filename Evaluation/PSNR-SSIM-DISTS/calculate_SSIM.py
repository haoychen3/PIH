...
for i in range(len(files1)):
    ref = prepare_image(Image.open(str(files1[i])).convert("RGB")).to(device)
    dist = prepare_image(Image.open(str(files2[i])).convert("RGB")).to(device)

    model = SSIM(channels=3)

    score = model(dist, ref, as_loss=False)

    ssim_list.append(score.item())

    print('score: %.4f' % score.item())

average_ssim = mean(ssim_list)
print('Average SSIM: %.4f' % average_ssim)